...
Want to output cleanly formated tables in an R Markdown document? Just add `print = TRUE` to any of the three `get_regression_` functions.
...
The resulting `submission.csv` is formatted such that in can be submitted on Kaggle, resulting in a "root mean squared logarithmic error" leaderboard score of 0.42919.
...
The three `get_regression` functions are merely wrappers of functions from the [`broom`](https://CRAN.R-project.org/package=broom/vignettes/broom.html){target="_blank"} package for converting statistical analysis objects into tidy tibbles:
...
* The `broom` package function names `tidy()`, `augment()`, and `glance()` don't mean anything to intro stats students, where as the `moderndive` package function names `get_regression_table()`, `get_regression_points()`, and `get_regression_summaries()` have more intuitive names.
