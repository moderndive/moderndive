...
-   New `View()` wrapper (resolves issue [#99](https://github.com/moderndive/moderndive/issues/99)). In an interactive R session it behaves identically to `utils::View()`. In non-interactive contexts (R Markdown, Quarto, scripts) where `utils::View()` typically errors, it instead renders an interactive `DT::datatable()` inline so documents can still knit/render. `DT` is now in `Imports`. A short `packageStartupMessage()` is emitted only on non-interactive attach to explain the override; interactive sessions see no extra message. Attaching `moderndive` masks `utils::View`.
