...
In Table \ref{tab:random-sample-courses} we present a subset of `r ncol(evals_sample)` of the `r ncol(evals)` variables included for a random sample of `r nrow(evals_sample)` courses^[For details on the remaining `r ncol(evals) - ncol(evals_sample)` variables, see the help file by running `?evals`.]. These include:
...
## Common approaches to teaching regression
...
Furthermore, recall that since all outputs in the `moderndive` package are tibble data frames, custom residual analysis plots can be created instead of relying on the default plots yielded by `plot.lm()`. For example, we can check for the noramality of residuals using the histogram of residuals shown in Figure \ref{fig:residuals-1}.
...
```{r kaggle-2, fig.cap="Resulting Kaggle RMSLE score."}
...
```{r interaction-and-parallel-slopes-model-1, echo=FALSE, fig.cap = "Interaction (left) and parallel slopes (right) models."}
...
```{r interaction-and-parallel-slopes-model-2, echo=FALSE, fig.cap = "Interaction (left) and parallel slopes (right) models."}
...
As we mentionned earlier, the three `get_regression` functions are merely wrappers of functions from the `broom` package for converting statistical analysis objects into tidy tibbles along with a few added tweaks, but with the introductory statistics student in mind [@R-broom]:
...
1. The variables included in the outputs of the above 3 `broom` functions are not all applicable to an introductory statistics students and of those that were we found them to be unintuitively named. We thereforme cut out some of the variables from the output and renamed some of the remaining variables. For example, compare the outputs of the `get_regression_points()` wrapper function and the parent `broom::augment()` function.
