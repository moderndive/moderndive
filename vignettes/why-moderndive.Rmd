...
1. Get a tidy regression table **with** confidence intervals:
    ...
    ```
2. Get information on each point/observation, including fitted/predicted values & residuals, in a single data frame:
    ```{r}
...
The three `get_regression_X()` functions are merely wrappers of functions from the [`broom`](https://CRAN.R-project.org/package=broom/vignettes/broom.html){target="_blank"} package for converting statistical analysis objects into tidy tibbles:
...
Many thanks to Max Kuhn [\@topepo](https://github.com/topepo){target="_blank"} for his helpful feedback for this vignette.
