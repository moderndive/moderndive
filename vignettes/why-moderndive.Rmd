...
The outcome variable of interest is `score`, which is the teaching evaluation score out of 5 as given by students. Let's fit a regression model of teaching `score` as a function of instructor `age` and analyze the output "the good old fashioned way", using `summary.lm()`:
...
Here are five common student comments/questions we've heard over the years in our intro stats courses:
...
**We think that the functions included in [`moderndive`](https://moderndive.github.io/moderndive/){target="_blank"}, an R package for tidyverse-friendly introductory linear regression, are effective pedagogical tools that can help address these five common student comments/questions. We now argue why.**
...
* The `Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1` only encourage **p-hacking**. In case you have not yet been convinced of the perniciousness of p-hacking, perhaps comedian [John Oliver can convince you](https://www.youtube.com/embed/0Rnq1NpHdmw){target="_blank"}.  
...
All the functions in the `moderndive` package return tidy tibbles! So for example, by piping the above `get_regression_table(score_model)` output into `knitr::kable()`, you can have aesthetically pleasing regression tables in R Markdown documents, instead of jarring computer output font:
...
Furthermore, recall that all outputs in the `moderndive` package are data frames/tidy tibbles, thus you can easily create custom residual analysis plots, instead of the default ones yielded by `plot(score_model)`:
...
<center>
![](kaggle.png){ width=500px }
</center>

Say you've downloaded the `train.csv` and `test.csv` training and test data available on the above Kaggle page. Here is a 9-line "baby's first kaggle competition" submission that will:
...
**TODO**: Implement [GitHub issue 31](https://github.com/moderndive/moderndive/issues/31){target="_blank"}
...
The three `get_regression_X()` functions are merely wrappers of functions from the [`broom`](https://cran.r-project.org/web/packages/broom/vignettes/broom.html){target="_blank"} package for converting statistical analysis objects into tidy tibbles:
...
Why did we take this approach to address the 5 points at the outset of the article?

* The function names `tidy()`, `augment()`, and `glance()` don't mean anything to intro stats students.
* The default column/variable names in the outputs of the above 3 functions are a little daunting for intro stats students to interpret. 
* By writing wrappers, instead of custom functions, there is minimal "re-inventing the wheel necessary." 
